...
async function checkAndProcessTelcoPosts() { // Renamed function to reflect both check and reply/repost
  ...
  for (const handle of telcoHandles) {
    try {
      ...
      const userTimeline = await userClient.v2.userTimeline(userId, {
        max_results: 5, // Fetch a few recent tweets to check for keywords
      });

      if (userTimeline.data && userTimeline.data.length > 0) {
        for (const tweet of userTimeline.data) { // Iterate through the few recent tweets
            ...
            if (!processedTweetIds.has(tweet.id)) {
                ...
                if (hasBuyDataKeyword) {
                    // --- Reply to the tweet ---
                    try {
                        const reply = await userClient.v2.reply(genericReplyText, tweet.id);
                        console.log(`Successfully replied to tweet ${tweet.id} from ${handle}`, reply);
                        processedTweetIds.add(tweet.id); // Mark as processed

                         // NOTE FOR PRODUCTION: Persist processedTweetIds to a database.

                    } catch (replyError) {
                        console.error(`Error replying to tweet ${tweet.id} from ${handle}:`, replyError);
                         if (replyError.code) {
                             console.error("X API Reply Error Code:", replyError.code);
                             console.error("X API Reply Error Message:", replyError.message);
                             if (replyError.errors) {
                                 console.error("X API Detailed Reply Errors:", replyError.errors);
                             }
                         }
                    }
                        ...
                        try {
                            ...
                            console.error(
                             ...
                             if (repostError.code) {
                                 ...
                                 console.error("X API Repost Error Message:", repostError.message);
                                 if (repostError.errors) {
                                     console.error("X API Detailed Repost Errors:", repostError.errors);
                                 }
                             }
                        }
                    } else {
                         console.log(`Tweet ${tweet.id} from ${handle} does not contain keyword and is not the most recent fetched. Skipping.`);
                    }
                }
            } else {
      ...
      console.error(`Error fetching timeline for ${handle}:`, fetchError);
       if (fetchError.code === 429) {
           console.warn(`Rate limit hit for ${handle}. Waiting before next request.`);
       }
       if (fetchError.code) {
...
const startRepostJob = () => { // Keep the function name for now, or rename to startProcessingJob
  if (!processTask) {
...
const triggerRepost = async (req, res) => { // Keep triggerRepost, but it now triggers the combined logic
  const userClient = getUserClient();
  if (!userClient) {
    return res
      .status(401)
