...
const Header = () => {
  ...
  return (
    <nav className="relative z-40 min-h-[76px] border-b border-line bg-paper px-4 py-4 pl-16 md:px-6 md:pl-6 xl:px-12" aria-label="Account controls">
      <div className="mx-auto flex max-w-app-main min-w-0 items-center justify-between gap-2 md:gap-5">
        <div className="min-w-0 text-[11px] text-muted md:text-xs">
          Your account <span className="ml-3 hidden text-ink md:inline">/&nbsp; {pageTitle}</span>
        </div>
        <div className="flex items-center gap-0.5 md:gap-2">
          ...
          <div className="relative" ref={dropdownRef}>
            <button className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-md text-muted hover:bg-tint hover:text-ink focus-visible:outline-2 focus-visible:outline-accent focus-visible:-outline-offset-2" onClick={toggleDropdown} aria-label="Profile options" aria-expanded={isDropdownOpen} aria-controls="account-profile"><FiUser className="h-[18px] w-[18px]" /></button>
            {isDropdownOpen && (
              <section id="account-profile" className="absolute right-[-4px] top-[calc(100%+12px)] z-50 w-[240px] max-w-[calc(100vw-16px)] overflow-hidden rounded-lg border border-line bg-paper text-ink shadow-[0_8px_24px_#0000000d] md:right-0 md:max-w-none" aria-label="Profile options">
                <div className="flex flex-col gap-[5px] overflow-wrap-anywhere border-b border-line p-[18px]">
                  <strong className="text-[13px] font-mediumish">{user?.username || 'Your account'}</strong>
                  {user?.email && <span className="text-[11px] text-muted">{user.email}</span>}
                </div>
                <div className="p-1.5">
                  <button className="flex min-h-11 w-full items-center gap-3 rounded px-3 py-2.5 text-left text-xs hover:bg-bg" onClick={() => { navigate("/settings"); setIsDropdownOpen(false); }}><FiUser className="text-base text-muted" />My profile</button>
