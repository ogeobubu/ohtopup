        ...
        <div
          ...
          className={`border border-solid rounded-lg cursor-pointer transition-all duration-200 ${
            activeTab === tab.name
              ...
              : "border-line text-muted hover:bg-gray-50 dark:hover:bg-gray-700"
          } flex w-full items-center space-x-3 md:space-x-4 px-3 md:px-4 py-3 md:py-4`}
          onClick={() => setActiveTab(tab.name)}
        >
          <tab.icon
            className={`w-4 h-4 md:w-5 md:h-5 flex-shrink-0 ${activeTab === tab.name ? "text-blue-500" : "text-muted"}`}
          />
          <div className="flex flex-col gap-0.5 min-w-0 flex-1">
            <span className="text-sm md:text-base font-medium truncate">{tab.label}</span>
            {tab.subtitle && (
              <small className="text-xs md:text-sm text-muted truncate">{tab.subtitle}</small>
            )}
          </div>
        </div>
      ))}
