...
const Data = () => {
  ...
  return (
    <div className="border border-solid border-line rounded-md p-4 md:p-6 h-full overflow-y-auto">
      {/* Tabs - Mobile responsive */}
      <div className="mb-4 flex rounded-lg border border-solid border-line bg-[#F7F9FB] overflow-hidden">
        <button aria-pressed={activeTab === "Set"}
          className={`min-h-[44px] flex-1 py-2 px-1 font-medium transition-colors duration-300 text-sm md:text-base ${
            activeTab === "Set"
              ? "text-green-500 bg-paper"
              ...
              <Form className="flex flex-col space-y-4">
                ...
                <div className="flex flex-col">
                  ...
                  {isLoadingIdentifiers ? (
                    ...
                    <div className="flex flex-wrap justify-center gap-2 md:gap-4 border border-solid border-line py-2 px-1 rounded">
                      {identifiers?.map((provider) => (
                        <button
                          key={provider?.serviceID}
                          type="button"
                          className={`min-h-[44px] flex flex-col items-center p-1 md:p-2 rounded-lg transition-all ${
                        ...
                        >
                          ...
                          />
                          <span className="text-xs mt-1 text-muted capitalize">
                            {provider?.serviceID}
                          </span>
                        </button>
                      ))}
                ...
                <div className="bg-[#F7F9FB] rounded-md p-2 md:p-4 w-full">
                  {isLoadingVariations ? (
                    ...
                    <div className="max-h-96 overflow-y-auto">
                      {variations.data.map((variation) => (
                        <div
                          ...
                          className="flex justify-between items-center p-2 md:p-3 border-b border-line hover:bg-gray-50 transition duration-200"
                        >
                          <label className="flex-1 text-ink text-sm md:text-base">
                            {variation.name}
                          </label>
                          <input
                            type="checkbox"
                    ...
                    <div className="flex flex-col items-center justify-center p-4 text-muted text-center">
                ...
                <button
                  className="min-h-[44px] bg-tint disabled:bg-gray-300 disabled:cursor-not-allowed w-full py-3 px-4 rounded-md text-white font-semibold transition-colors duration-200 flex items-center justify-center"
                  type="submit"
                      ...
                      {getVariations.data.map((variation) => (
                        ...
                        >
                          ...
                          <button
                            ...
                            className={`min-h-[44px] relative h-5 w-10 rounded-full transition-colors focus:outline-none ${
                              variation.isActive ? 'bg-tint' : 'bg-gray-300'
                            }`}
                          >
                            <span
                            ...
                            >
                              {isToggling === variation.variation_code ? (
                                ...
                                <FiX className="h-3 w-3 text-muted" />
                              )}
                            </span>
                          </button>
                        </div>
