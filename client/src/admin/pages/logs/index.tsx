...
const SystemLogs = () => {
  ...
  return (
    <div className={`p-2 md:p-6 ${isDarkMode ? "bg-gray-900 text-white" : "bg-bg text-ink"} min-h-screen`}>
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-4 md:mb-8">
          <div className="flex items-center space-x-2 md:space-x-3 mb-2 md:mb-4">
            ...
            <h1 className="text-[26px] font-mediumish leading-tight tracking-[-0.6px] text-ink text-xl md:text-3xl font-bold">System Logs</h1>
          </div>
          <p className={`text-xs md:text-sm ${isDarkMode ? "text-muted" : "text-muted"}`}>
            Monitor system activities, errors, and user actions
          </p>
        </div>

        {/* Filters and Search */}
        <div className={`mb-4 md:mb-6 p-3 md:p-4 rounded-lg ${isDarkMode ? "bg-gray-800" : "bg-paper"} `}>
          <div className="flex flex-col gap-3 md:gap-4">
            {/* Search */}
            <div className="w-full">
              <div className="relative">
                <FaSearch className="absolute left-3 top-2.5 md:top-3 text-muted text-sm md:text-base" />
                <input
                  type="text"
                  ...
                  className={`w-full pl-9 md:pl-10 pr-4 py-2 text-sm md:text-base border rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent ${
                    isDarkMode
                      ? "bg-gray-700 border-line text-white placeholder-gray-400"
                      : "bg-paper border-line text-ink placeholder-gray-500"
                  }`}
                />
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <div className="flex items-center space-x-2 flex-1">
                <FaFilter className="text-muted text-sm md:text-base" />
                <select
                  value={filterType}
              ...
              {/* Export Button */}
              <button
                className="min-h-[44px] flex items-center justify-center space-x-2 px-3 md:px-4 py-2 bg-accent text-white rounded-lg transition-colors text-sm md:text-base"
              >
        ...
        {/* Loading State */}
        {loading && (
          <div className={`text-center py-8 md:py-12 ${isDarkMode ? "text-muted" : "text-muted"}`}>
        ...
        {!loading && !error && (
          <div className={`rounded-lg overflow-hidden ${isDarkMode ? "bg-gray-800" : "bg-paper"}`}>
            <div className="overflow-x-auto">
              <table className="w-full border-collapse bg-paper text-ink text-xs w-full">
                <thead className={`${isDarkMode ? "bg-gray-700" : "bg-bg"}`}>
                  <tr>
                ...
                <tbody className={`divide-y ${isDarkMode ? "divide-gray-700" : "divide-gray-200"}`}>
                  {currentLogs.map((log) => (
                    <tr key={log.id} className={`${isDarkMode ? "hover:bg-gray-700" : "hover:bg-gray-50"}`}>
                      ...
                      <td className="px-3 md:px-6 py-3 md:py-4 whitespace-nowrap text-xs md:text-sm">
                        <button className="min-h-[44px] text-green-600 hover:text-green-800 dark:text-green-400 dark:hover:text-green-300 p-1">
          ...
          {/* Pagination */}
          {totalPages > 1 && (
            <div className={`px-3 md:px-6 py-2 md:py-3 ${isDarkMode ? "bg-gray-700" : "bg-bg"} flex flex-col sm:flex-row items-center justify-between gap-2`}>
              <div className={`text-xs md:text-sm ${isDarkMode ? "text-muted" : "text-ink"} text-center sm:text-left`}>
              ...
              <div className="flex space-x-1 md:space-x-2">
                <button
                  ...
                  className="min-h-[44px] px-2 md:px-3 py-1 text-xs md:text-sm border rounded disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors"
                >
                  Prev
                </button>
                <span className={`px-2 md:px-3 py-1 text-xs md:text-sm ${isDarkMode ? "text-gray-300" : "text-ink"}`}>
