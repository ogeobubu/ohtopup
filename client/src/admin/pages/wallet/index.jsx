...
import { FaPlus, FaWallet, FaToggleOn, FaToggleOff } from "react-icons/fa";
import {
  depositWallet,
  ...
  const handleAddFunds = async (amount, resetForm) => {
    if (selectedUser) {
      try {
        ...
        toast.success(`Successfully added ₦${amount} to ${selectedUser.username}'s wallet.`);
        resetForm();
        refetchWallets();
        closeModal();
      } catch (error) {
  ...
  return (
    <>
      ...
      <div className="mb-3 flex rounded-lg border border-solid max-w-xs border-gray-300 bg-[#F7F9FB] py-1 px-1">
        <button
          className={`py-1 px-1 font-medium transition-colors duration-300 ${
            activeTab === "Wallets" ? "text-blue-500 bg-white rounded-lg w-40" : "text-gray-500 hover:text-gray-800 w-40"
          }`}
        ...
        <button
          className={`py-1 px-1 font-medium transition-colors duration-300 ${
            activeTab === "Transactions" ? "text-blue-500 bg-white rounded-lg w-40" : "text-gray-500 hover:text-gray-800 w-40"
          }`}
                ...
                render: (row) => (
                  <div className="flex space-x-2">
                    <button
                      ...
                      className={`flex justify-center items-center rounded-full border border-solid w-6 h-6 ${row.isActive ? "border-green-500" : "border-red-500"} text-blue-500`}
                    >
                      {row.isActive ? <FaToggleOn className="text-green-500" size={15} /> : <FaToggleOff className="text-red-500" size={15} />}
                    </button>
                    <button
      ...
      {activeTab === "Transactions" && (
        <div className="overflow-x-auto">
          {loadingTransactions ? (
            ...
            <Table
              columns={[
                { header: "User", render: (row) => <p>{row.user.username}</p> },
                { header: "Amount", render: (row) => <p>₦{row.amount.toFixed(2)}</p> },
                { header: "Type", render: (row) => <p>{row.type}</p> },
                { header: "Timestamp", render: (row) => <p>{new Date(row.timestamp).toLocaleString()}</p> },
      ...
      <Modal
        ...
        title={isCreatingWallet ? `Create Wallet for ${selectedUser?.username}` : `Add Funds to ${selectedUser?.username}`}
      >
        <Formik
        ...
        >
          {({ errors, touched }) => (
            <Form className="flex flex-col">
              {!isCreatingWallet ? (
                <>
                  <label htmlFor="amount" className="mb-1">Enter Amount:</label>
                  <Field
                    name="amount"
                    as={Textfield}
                    placeholder="₦0.00"
                ...
                <p>Are you sure you want to create a wallet for {selectedUser?.username}?</p>
              )}
              <Button
                type="submit"
