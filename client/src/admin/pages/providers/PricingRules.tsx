...
export default function PricingRules() {
  ...
  const field = (name: keyof typeof initial, label: string, options?: string[]) => <label className="block" key={name}>
    <span className="mb-2 block text-xs font-mediumish text-ink">{label}</span>
    {options ? <select className="block min-h-[46px] w-full rounded-[5px] border border-line bg-paper px-3 py-2.5 text-sm text-ink" value={form[name]} onChange={e => set(name, e.target.value)}>{options.map(value => <option key={value} value={value}>{value === '*' ? 'All networks/services' : value}</option>)}</select>
      : <input className="block min-h-[46px] w-full rounded-[5px] border border-line bg-paper px-3 py-2.5 text-sm text-ink" value={form[name]} type={['providerCommissionRate', 'providerCommissionCap', 'customerDiscountRate'].includes(name) ? 'number' : 'text'} min="0" step="0.01" max={name === 'customerDiscountRate' || (name === 'providerCommissionRate' && form.providerCommissionType === 'percentage') ? '99.99' : undefined} required={name !== 'planCode' && name !== 'providerCommissionCap'} onChange={e => set(name, e.target.value)} />}
  </label>;
  ...
  return <section className="space-y-5">
    <div><h2>OhTopUp pricing rules</h2><p className="text-muted mt-2">Configure how provider commission is shared between customers and your platform. Enter the provider's contracted commission rate, then set the customer discount. OhTopUp retains the difference as platform margin before operational fees.</p></div>
    {query.isError && <p role="alert">Could not load pricing rules. <button onClick={() => query.refetch()}>Retry</button></p>}

    {previewData && <div className="min-w-0 overflow-hidden rounded-lg border border-line bg-paper p-4">
    ...
    {(providerRate > 0 || discount > 0) && <div className="min-w-0 overflow-hidden rounded-lg border border-line bg-paper p-4">
      ...
      <p className="text-xs text-muted mt-2">Provider commission {form.providerCommissionType === 'flat' ? `is ₦${providerRate} per transaction` : `is ${providerRate}%`}{form.providerCommissionCap !== '' ? `, capped at ₦${form.providerCommissionCap}` : ''}. Customer discount is {discount}%. OhTopUp retains the ₦{platformRate} difference before operational fees.</p>
    </div>}

    <form className="min-w-0 overflow-hidden rounded-lg border border-line bg-paper p-4 sm:p-6 space-y-4" onSubmit={e => { e.preventDefault(); setSaved(false); mutation.mutate({ ...form, providerCommissionRate: Number(form.providerCommissionRate), providerCommissionCap: form.providerCommissionCap === '' ? null : Number(form.providerCommissionCap), customerDiscountRate: Number(form.customerDiscountRate) }); }}>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {field('provider', 'Third-party provider', ['vtpass', 'clubkonnect'])}
        {field('service', 'Service', ['airtime', 'data', 'cable', 'electricity'])}

        <label className="block">
          <span className="mb-2 block text-xs font-mediumish text-ink">Network or service ID</span>
          <select className="block min-h-[46px] w-full rounded-[5px] border border-line bg-paper px-3 py-2.5 text-sm text-ink" value={form.network} onChange={e => set('network', e.target.value)}>
            <option value="*">All networks/services</option>
            {(options.data?.networks || []).map(n => <option key={n} value={n}>{n}</option>)}
          </select>
          {options.isLoading && <p className="text-xs text-muted mt-1">Loading networks…</p>}
        </label>

        {(form.service === 'data' || form.service === 'cable') && <label className="block">
          <span className="mb-2 block text-xs font-mediumish text-ink">Plan code (optional override)</span>
          <select className="block min-h-[46px] w-full rounded-[5px] border border-line bg-paper px-3 py-2.5 text-sm text-ink" value={form.planCode} onChange={e => set('planCode', e.target.value)}>
            <option value="">None — apply to all plans</option>
            {filteredPlanCodes.map(p => <option key={p.planId} value={p.planId}>{p.label}</option>)}
          </select>
          {filteredPlanCodes.length === 0 && form.network !== '*' && !options.isLoading && <p className="text-xs text-muted mt-1">No plans found for this network</p>}
        </label>}
      ...
      <p className="text-sm text-muted">Provider commission is what the third-party provider (VTPass/ClubKonnect) pays you per transaction. Customer discount is what you pass to the customer. OhTopUp margin is what your platform retains before operational fees.</p>
      <p className="text-sm text-muted">Use mtn, airtel, glo, or 9mobile for mobile services; the service ID (such as ikeja-electric or dstv) for bills. A plan rule takes priority over a network rule, then the provider/service default. Saving the same scope replaces its rates.</p>
      <p className="text-sm text-muted">Flat and capped commissions are checked against each purchase amount. Provider costs are estimates until confirmed by the delivery response. Keep these rates current with your provider contract.</p>
      {mutation.isError && <p role="alert" className="mt-1 text-[11px] text-danger">{mutation.error.message}</p>}
      {saved && <p role="status">Pricing rule saved.</p>}
      <div className="flex gap-2">
        <button className="inline-flex min-h-[46px] items-center justify-center gap-4 rounded-md border border-transparent bg-accent px-[19px] py-[11px] text-sm font-semibold text-white transition border border-transparent bg-accent text-white hover:bg-accent-dark" disabled={mutation.isPending}>{mutation.isPending ? 'Saving…' : isEditingSavedRule ? 'Update rule' : 'Save new rule'}</button>
        {isEditingSavedRule && <button type="button" className="inline-flex min-h-[46px] items-center justify-center gap-4 rounded-md border border-transparent bg-accent px-[19px] py-[11px] text-sm font-semibold text-white transition border border-line bg-paper text-ink hover:bg-tint" onClick={() => { setEditingId(null); setForm(initial); }}>Cancel</button>}
      </div>
    </form>

    {query.data && <>
      <div className="min-w-0 overflow-hidden rounded-lg border border-line bg-paper p-4"><h3>Delivered transactions using recorded pricing</h3><div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-2"><div><p className="text-sm text-muted">Customer discounts</p><p className="font-semibold">{formatNairaAmount(query.data.summary.customerDiscounts)}</p></div><div><p className="text-sm text-muted">Confirmed platform margin</p><p className="font-semibold">{formatNairaAmount(query.data.summary.confirmedMargin)}</p></div><div><p className="text-sm text-muted">Awaiting provider cost</p><p className="font-semibold">{query.data.summary.unconfirmedTransactions} transactions</p></div></div></div>
      <div className="min-w-0 w-full overflow-x-auto rounded-md border border-line bg-paper"><table className="w-full border-collapse text-xs"><thead><tr><th>Scope</th><th>Provider commission</th><th>Customer discount</th><th>OhTopUp margin</th><th>Action</th></tr></thead><tbody>{query.data.rules.map(rule => {
        ...
        return <tr key={rule._id}>
          ...
          <td><button className="inline-flex min-h-[46px] items-center justify-center gap-4 rounded-md border border-transparent bg-accent px-[19px] py-[11px] text-sm font-semibold text-white transition border border-line bg-paper text-ink hover:bg-tint" onClick={() => startEdit(rule)}>Edit</button></td>
        </tr>;
      })}</tbody></table></div>
