...
const Services = () => {
    ...
    },
    {
      ...
      render: (service) => (
        <div className="flex space-x-2">
          <button
            className="min-h-[44px] border border-solid border-green-500 flex justify-center items-center rounded-full w-6 h-6 text-green-500 hover:text-green-700"
            onClick={() => handleEditService(service)}
          >
            <FaEdit size={15} />
          </button>
          <button
            className="min-h-[44px] border border-solid border-red-500 flex justify-center items-center rounded-full w-6 h-6 text-red-500 hover:text-red-700"
  ...
  return (
    <div className="md:p-6 p-2 border border-solid rounded-md border-line w-full">
      ...
      <Modal isOpen={isOpen} closeModal={toggleModal} title="Create Service">
        <form onSubmit={formik.handleSubmit} className="space-y-4">
          ...
          <div className="flex items-center justify-between mb-3 w-full">
            <label
              className={`mr-2 ${isOn ? "text-green-600" : "text-muted"}`}
            >
              Is this available?
            </label>
            <div
            ...
            >
              <span
                className={`absolute left-0 w-6 h-6 bg-paper rounded-full transform transition-transform duration-200 ${
