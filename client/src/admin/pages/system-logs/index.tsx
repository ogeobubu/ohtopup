...
const SystemLogs = () => {
  ...
  const getLevelIcon = (level) => {
    switch (level) {
      ...
      default:
        return <FaInfo className="text-muted" />;
    }
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (log) => (
        <div className="max-w-xs lg:max-w-md xl:max-w-lg">
          <div className="text-sm font-medium text-ink truncate" title={log.message}>
            {log.message}
          </div>
    ...
    },
    {
      ...
      render: (log) => (
        <div className="text-sm min-w-0">
          {log.userEmail ? (
            <div>
              <div className="font-medium text-ink truncate" title={log.userEmail}>
                {log.userEmail}
              </div>
              {log.userId && (
    ...
    },
    {
      header: "IP & Agent",
      render: (log) => (
        <div className="text-xs text-muted max-w-32 hidden lg:block">
          {log.ipAddress && (
    ...
    },
    {
      ...
      render: (log) => (
        <button
          className="min-h-[44px] bg-accent text-white rounded-lg px-3 py-2 text-sm font-medium transition-colors whitespace-nowrap"
          onClick={() => {
  ...
  return (
    <div className="my-3 md:my-5 p-2 md:px-4 sm:px-6 lg:px-2">
      <div className="mb-4 md:mb-6">
        <h1 className="text-[26px] font-mediumish leading-tight tracking-[-0.6px] text-ink text-xl md:text-3xl font-bold text-ink mb-2">System Logs</h1>
        <p className="text-muted text-sm md:text-base">Monitor system activities, errors, and user actions</p>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 xl:grid-cols-6 gap-3">
          ...
          <div className="relative sm:col-span-2 lg:col-span-2 xl:col-span-2">
            <input
              ...
              className="border border-line rounded-lg px-3 md:px-4 py-2 pl-8 md:pl-10 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent w-full text-sm"
            />
            <FaSearch className="absolute left-2 md:left-3 top-2.5 md:top-3 h-3 w-3 md:h-4 md:w-4 text-muted" />
          </div>
          ...
          <div className="sm:col-span-2 lg:col-span-1 xl:col-span-1">
            <button
              ...
              className="min-h-[44px] bg-tint px-3 md:px-4 py-2 rounded-lg transition-all duration-200 transform text-xs md:text-sm w-full h-9 flex items-center justify-center"
            >
      ...
      {isLogsLoading ? (
        <div className="flex flex-col items-center justify-center py-12">
          ...
          <h3 className="text-lg font-semibold text-ink mb-2">Loading Logs</h3>
